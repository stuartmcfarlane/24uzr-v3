...
const polygon2lines = (polygon: Polygon) => {
...
const lineIntersection = ([a1, a2]: Line, [b1, b2]: Line) => {
...
const _splitArray = pred => a => a.reduce(
    ([has, hasnt], o) => {
        const yes = pred(o)
        return [
            [...(yes ? [...(has || []), o] : has)],
...
const splitArray = (...preds) => a => {
    if (preds.length === 0) return [a]
    if (a.length === 0) return [[],...splitArray(...preds.slice(1))([])]
    const [yes, no] = _splitArray(preds[0])(a)
    const splits = [yes, ...splitArray(...preds.slice(1))(no)]
    return splits
}
...
const mergeColinear = (lines: Line[]) => {
